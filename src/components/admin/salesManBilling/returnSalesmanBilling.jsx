...
const ReturnSalesmanBilling = () => {
    const params = useParams();
    const history = useNavigate();
    document.title = 'Retun Salesman Billing'
    CheckPageAccess();
    ...
    const updateRetunedSatus = () => {
        ...
        Axios.post(`${process.env.REACT_APP_API_SERVICE}/salesmanBilling/update`, model)
            .then((result) => {
                console.log('-updateRetunedSatus----success--:', result);
            })
            .catch((error)=> {
                console.log('-updateRetunedSatus----error--:', error);
            })
    }
    ...
    const handleSubmit = (e) => {
        ...
        const model = {
            ...
            total_selling: parseInt(formData.total_selling),
            commission_percent: parseInt(formData.commission_percent),
            our_commision: parseInt(formData.our_commision),
        }
    ...
    const getData = () => {
        ...
        .then((result) => {
            ...
            setFormData((prev) => {
                return {
                    ...prev,
                    salesman_id: result.data.salesman_id,
    ...
    return (
        ...
        <Typography variant="h2.heading" component="h2" className="page-heading">Return Salesman Billing</Typography>
        <Paper elevation={4} className="paper">
            <Grid container>
                ...
                <Grid item container xs={12} sm={12} md={6}>
                    ...
                    <Grid item xs={6} sm={6} md={6} style={{padding: '0.5rem 1rem 0 1rem'}} display="flex" justifyContent="flex-end">
                        <Typography variant="h2.heading" component="h2" style={{fontSize: '18px', fontWeight: '500'}}>
                            {formData.products.length > 0 ? (`Rs. ${numberWithCommas(totalPrice)}`) : ``}
                        </Typography>
                    </Grid>
                    ...
                    <Grid item xs={6} sm={6} md={6} style={{padding: '0.5rem 1rem 0 1rem'}} display="flex" justifyContent="flex-end" alignItems="center">
                        <TextField
                            ...
                            id="outlined-error-helper-text"
                            color="secondary"
                            onChange={(e) => setCommisionPercent(Math.abs(e.target.value))}
                    ...
                    <Grid item xs={6} sm={6} md={6} style={{padding: '0.5rem 1rem 0 1rem'}} display="flex" justifyContent="flex-end">
                        <Typography variant="h2.heading" component="h2" style={{fontSize: '14px', fontWeight: '400'}}>
                            {formData.products.length > 0 ? (`Rs. ${numberWithCommas((totalPrice*(100-commisionPercent))/100)}`) : `Total Rs. 0`}
                        </Typography>
                    </Grid>
                    ...
                    <Grid item xs={6} sm={6} md={6} style={{padding: '0.5rem 1rem 0 1rem'}} display="flex" justifyContent="flex-end">
                        <Typography variant="h2.heading" component="h2" style={{fontSize: '18px', fontWeight: '500'}}>
                            {formData.products.length > 0 ? (`Rs. ${numberWithCommas((totalPrice*commisionPercent)/100)}`) : `Total Rs. 0`}
                        </Typography>
                    </Grid>
                </Grid>
                ...
                <Stack direction="row" spacing={2} mt={2} justifyContent="flex-end">
                    ...
                    <Button 
                        ...
                        color="secondary" 
                        className="primary-button-dark"
                        onClick={() => { history('../salesman-billing/1') }}
                        >Cancel
                    </Button>
                </Stack>
                </Grid> 
...
export default ReturnSalesmanBilling;