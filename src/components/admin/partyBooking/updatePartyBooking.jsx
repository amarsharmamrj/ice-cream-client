...
const UpdatePartyBooking = () => {
    const params = useParams();
    const history = useNavigate();
    document.title = 'Update Party Booking'
    CheckPageAccess();
        ...
        if(targetName === 'advance'){
            ...
            setFormData(prev => {
                return {
                    ...
                    final_price: (formData.total_price - formData.total_discount) - formData.advance
                }
            })
            // calculateTotalPrice();
        }else {
    ...
    const handleProductDiscountedRate = (e) => {
        ...
        if(targetValue != '' || targetValue != '0'){
            ...
            setFormData((prev) =>{
                return {
                    ...prev,
                    final_price: formData.total_price - formData.total_discount
                }
            })
            calculateTotalPrice();
        }
    ...
    const handleSubmit = (e) => {
        ...
        if(flag){
            Axios.post(`${process.env.REACT_APP_API_SERVICE}/partyBooking/update`, model)
            .then((result) => {
    ...
    const getData = () => {
        Axios.get(`${process.env.REACT_APP_API_SERVICE}/partyBooking/getById/` + params.id)
        .then((result) => {
            ...
            setFormData((prev) => {
                return {
                    ...
                    alternate_mobile_number: result.data.alternate_mobile_number,
    ...
    return (
        ...
        <Typography variant="h2.heading" component="h2" className="page-heading">Update Party Booking</Typography>
                    ...
                    <>
                        ...
                        <Grid item xs={12} sm={12} md={12} style={{padding: '0.5rem 1rem 0 1rem'}} display="flex" justifyContent="flex-end">
                            <Typography variant="h2.heading" component="h2" style={{fontSize: '18px', fontWeight: '400'}}>
                                {formData.products.length > 0 ? (`Total: Rs. ${numberWithCommas(formData.total_price)}`) : `Total Rs. 0`}
                            </Typography>
                        </Grid>

                        <Grid item xs={12} sm={12} md={12} style={{padding: '0.5rem 1rem 0 1rem'}} display="flex" justifyContent="flex-end">
                            <Typography variant="h2.heading" component="h2" style={{fontSize: '18px', fontWeight: '400'}}>
                                {formData.products.length > 0 ? (`Total Discount(-): Rs. ${numberWithCommas(formData.total_discount)}`) : `Total Discount Rs. 0`}
                            </Typography>
                        </Grid>
                        ...
                        <Grid item xs={12} sm={12} md={12} className="input-padding" display="flex" justifyContent="flex-end">
                            <Typography variant="h2.heading" component="h2" style={{fontSize: '20px', fontWeight: '500'}}>
                                {formData.products.length > 0 ? (`Final Price: Rs. ${numberWithCommas(formData.final_price)}`) : `Total Discount Rs. 0`}
                            </Typography>
                        </Grid>
                    </>
                ) : '' }
...
export default UpdatePartyBooking;